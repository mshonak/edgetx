...
static led_timer_value_t _led_dma_buffer[WS2821_DMA_BUFFER_LEN];
...
{
  return tc * WS2812_BYTES_PER_LED * _led_strip_len;
}
...
static void _update_dma_buffer(const stm32_pulse_timer_t* tim, uint8_t tc)
{
  ...
  if (_led_seq_cnt < _led_strip_len) {
    ...
    if (_led_seq_cnt < _led_strip_len + 2) {
      auto len = WS2812_BYTES_PER_LED * _led_strip_len;
      auto offset = _calc_offset(tc);
      memset(&_led_dma_buffer[offset], 0, len * sizeof(led_timer_value_t));
    }
    _led_seq_cnt++;
...
static void _init_timer(const stm32_pulse_timer_t* tim)
{
  ...
  auto len = WS2812_BYTES_PER_LED * _led_strip_len * 2;
  LL_DMA_SetDataLength(tim->DMAx, tim->DMA_Stream, len);
